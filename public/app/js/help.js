...
const HELP = {
  jobTypes: {
    ...
    items: [
      ...
      { h: 'Photo set', rate: 'cart', d: 'A batch delivered together — add each piece into one entry. Its combined units stay on a single line, so an over-scope set is obvious without a quota. Revise the whole set as one piece.',
        list: ['Add a row per piece (Banner, Infographic, Variation…) with its quantity', 'Total = the pieces added together', 'A revision applies the severity to the whole set — and counts it as one piece in client difficulty'] },
